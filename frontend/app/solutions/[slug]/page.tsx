      ...
      <main>
        <SimplePageHero eyebrow="Solution" title={solution.title} text={solution.description}>
          <div className="mt-6 flex justify-center">
            <span className="inline-flex items-center gap-2 rounded-full border border-[#0f766e]/20 bg-white px-4 py-2 text-sm font-semibold text-[#0f766e]">
        ...
        <section className="mx-auto grid max-w-7xl gap-5 px-4 pb-20 sm:px-6 lg:grid-cols-3 lg:px-8">
          <article className="rounded-xl border border-black/10 bg-white p-6 shadow-sm">
            ...
            <ul className="mt-4 space-y-3 text-sm leading-6 text-[#52615d]">
          ...
          <article className="rounded-xl border border-black/10 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {solution.modules.map((item) => <span className="rounded-full bg-[#0f766e]/10 px-3 py-1 text-sm font-semibold text-[#0f766e]" key={item}>{item}</span>)}
            </div>
          </article>
          <article className="rounded-xl border border-black/10 bg-white p-6 shadow-sm">
