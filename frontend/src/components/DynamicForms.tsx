...
export function DynamicForms({ token }: DynamicFormsProps) {
  ...
  const selectedForm = useMemo(
    () => allForms.find((form) => form.id === selectedFormId) ?? allForms[0],
  ...
  const publishMutation = useMutation({
    ...
    onSuccess: async (savedForm, variables) => {
      ...
      pushToast({
        title:
          savedForm.status === "published" ? "Form published" : "Form saved",
  ...
  const serverCompatibilityQuery = useQuery({
    ...
    enabled: Boolean(
      token && token !== "preview-token" && isPersistedSelectedForm,
    ),
  });
  ...
  function createBlankForm() {
    const nextForm = templateToForm({
      ...
      description:
        "A blank form with core respondent, GPS, and review questions.",
      fields: 6,
      minutes: 5,
      popularity: 0,
    ...
    setBuilderResult(
      `${nextForm.name} was created with ${nextForm.fields.length} starter questions. Rename the form and adjust required fields before publishing.`,
    );
    pushToast({
      title: "Blank form created",
      description: "Start editing questions and publish when ready.",
      tone: "success",
    });
  }
  ...
  function addCatalogField(type: FieldType) {
    ...
    setBuilderResult(
      `${field.label} was added. Edit the label, required setting, and rules in the properties panel.`,
    );
  }
  ...
  return (
    <section aria-labelledby="forms-title" className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">
            Build clear, offline-ready forms your field team can use confidently
            on mobile devices.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            onClick={() => {
              setBuilderResult(
                "Import files are handled in Data tools so validation, column mapping, versioning, and rollback stay in one governed workflow.",
              );
              pushToast({
                title: "Opening Data tools",
                description:
                  "Review XLSForm, JSON, CSV, and Kobo/ODK migration files in Data tools.",
                tone: "neutral",
              });
              setActiveView("data");
            }}
          ...
          <Button onClick={createBlankForm} type="button" variant="primary">
      ...
      {!allForms.length && builderMode !== "templates" ? (
        <section className="rounded-lg border bg-panel p-6 text-center">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            ...
            <Button
              onClick={() => {
                setBuilderResult(
                  "Opening Data tools for governed file import, validation, column mapping, and rollback.",
                );
                setActiveView("data");
              }}
              type="button"
        ...
        <div
          className={cn(
            "grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)] 2xl:grid-cols-[280px_minmax(0,1fr)_360px]",
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Badge tone="accent">
                        Live version {selectedForm.activeVersion}
                      </Badge>
                    </div>
                    <p className="mt-1 text-xs text-muted-foreground">
                      Offline-ready · updated{" "}
              ...
              {selectedFormCompatibility && selectedFormWorkbook ? (
                <section
                  ...
                  aria-label="Collection compatibility"
                >
                  {[
                    [
                      "XLSForm",
                ...
                <section className="rounded-lg border bg-panel p-4">
                  <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">
                        Atlas can package this form into survey, choices, and
                        settings sheets for Kobo/ODK-style migration review.
                      </p>
                      <div className="mt-4 grid grid-cols-3 gap-2">
                        {[
                          ...
                          [
                            "Choices",
                            xlsFormQuery.data?.choices.length ??
                              selectedFormWorkbook.choices.length,
                          ],
                          [
                            "Version",
                            xlsFormQuery.data?.settings.version ??
                              selectedFormWorkbook.settings.version,
                          ],
                        ].map(([label, value]) => (
                          <div
                    ...
                    <div>
                      ...
                      <div className="mt-3 space-y-2">
                        {[
                          [
                            "Mobile collection",
                            "Offline capture with GPS, media, repeat groups, and retry sync.",
                          ],
                          [
                            "Web forms",
                            "Browser-based collection for desktop or shared devices.",
                          ],
                          [
                            "Public link review",
                            "Prepare controlled external access after sharing rules are configured.",
                          ],
                        ].map(([title, text]) => (
                          <div
                            className="rounded-md border bg-background p-3"
                  ...
                  {(activeCompatibility?.warnings ?? []).length ? (
                    <div className="mt-4 rounded-md border border-warning/25 bg-warning/10 p-3">
                      ...
                      <ul className="mt-2 space-y-1 text-xs leading-5 text-muted-foreground">
                        {(activeCompatibility?.warnings ?? []).map(
                          (warning) => (
              ...
              >
                <div className="flex items-center justify-between border-b px-4 py-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Drag questions to reorder. Select one to edit details and
                      rules.
                    </p>
                  </div>
                  <Badge tone="accent">
                    {selectedForm.fields.length} questions
                  </Badge>
                </div>
                <DndContext
                  sensors={sensors}
                ...
                >
                  <SortableContext
                    items={selectedForm.fields.map((field) => field.id)}
              ...
              <section className="rounded-lg border bg-panel p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Check how this form will feel for a field officer.
                    </p>
                  </div>
                ...
                <div
                  className={cn(
                    "mt-4 rounded-xl border bg-background p-4",
                ...
                <div className="mt-4 space-y-3">
                  {[
                    [
                      ...
                      "Only show it after a specific earlier answer.",
                    ],
                    [
                      "Make it required when",
                      "Ask for an answer only in situations that need it.",
                    ],
                    [
                      "Check the answer",
                      "Set safe ranges, formats, or accuracy limits.",
                    ],
                    [
                      "Calculate a value",
                      "Add totals or scores from previous answers.",
                    ],
                  ].map(([label, helper]) => (
                    <div
                      key={label}
