...
def test_openapi_is_versioned_under_api_v1() -> None:
    ...
    assert "/api/v1/platform/summary" in response.json()["paths"]
    assert "/api/v1/platform/users" in response.json()["paths"]
    assert "/api/v1/platform/audit-logs" in response.json()["paths"]
    assert "/api/v1/platform/roles" in response.json()["paths"]
    assert "/api/v1/platform/feature-flags" in response.json()["paths"]
    assert "/api/v1/platform/system-health" in response.json()["paths"]
    assert "/api/v1/platform/security" in response.json()["paths"]
    assert "/api/v1/platform/integrations" in response.json()["paths"]
